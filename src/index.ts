 ...
 * Create a new sign
 */
export function sign<T=any> (value?:T):{ sign, effect, computed, Sign } {
     ...
     */
    function effect<T=any> (fn:()=>T):()=>void {
    ...
    // signal gets updated
    function computed<T=any> (fn:()=>T) {
        const derived = sign<T>().sign
        effect(() => {
            derived.value = fn()
        })
        return derived
    ...
    return { sign: signal, effect, computed, Sign }
}
